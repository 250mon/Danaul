...
class TreatmentModel(DataModel):
    ...
    def init_params(self):
        self.set_table_name('treatments')

        self.col_edit_lvl = {
            'treatment_id': EditLevel.NotEditable,
            'active': EditLevel.AdminModifiable,
            'treatment_name': EditLevel.AdminModifiable,
    ...
    def get_default_delegate_info(self) -> List[int]:
        ...
        :return:
        """
        default_info_list = [self.get_col_number(c) for c in ['treatment_name', 'description']]
    ...
    def data(self, index: QModelIndex, role=Qt.DisplayRole) -> object:
        ...
        if role == Qt.DisplayRole or role == Qt.EditRole or role == self.SortRole:
            int_type_columns = ['treatment_id', 'category_id']
    ...
    def validate_new_row(self, index: QModelIndex) -> bool:
        """
        This is used to validate a new row generated by Singletreatments.indow
        when the window is done with creating a new row and emits add_treatments.signal
        :param index:
        :return:
        """
        treatment_name_col = self.get_col_number('treatment_name')
        new_treatment_name = index.siblingAtColumn(treatment_name_col).data()
