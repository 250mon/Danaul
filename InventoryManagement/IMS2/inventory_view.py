...
class InventoryWindow(QMainWindow):
    start_signal = Signal(int, pd.DataFrame)
    ...
    def __init__(self):
        ...
        self.logs = Logs()
        self.logger = self.logs.get_logger('inventory_view')
    ...
    async def update_db(self, action: str, df: pd.DataFrame):
        if action == "update_items":
            self.logger.debug('Updating DB ... update_items')
            print(self.item_model.get_changes())
            await self.lab.di_db.insert_items_df(self.item_model.get_changes())
